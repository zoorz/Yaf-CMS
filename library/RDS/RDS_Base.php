 ...
 * @package RDS
 */
require_once 'RDS_Dummy.php';
require_once 'RDS_Profiler.php';
...
abstract class RDS_Base extends RDS_Dummy
{
	...
	public function dumpStackTrace( $limit = 3, $stack_level = 1 )
	{
		...
		for( $current = $stack_level; $current < $lastFrame; $current++ )
		{
			...
			$file = array_key_exists( 'file', $stack[$current] ) ? $stack[$current]['file'] : 'unknown file';
			$line = array_key_exists( 'line', $stack[$current] ) ? $stack[$current]['line'] : 'unknown line';

			$stackItem = $current - $stack_level;
			$this->debug( RDS::DEBUG, "[{$stackItem}] --> {$file}({$line}){$origin}", $stack[$stack_level]['file'], $stack[$stack_level]['line'] );
		}
	}
	/**
	 * Create new profiler.
